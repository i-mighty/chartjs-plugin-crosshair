...
export default {
  ...
  drawTracePoints: function (chart) {
    for (
      ...
      const radius = this.getOption(chart, "point", "radius");
      const borderColor = this.getOption(chart, "point", "borderColor");
      const borderWidth = this.getOption(chart, "point", "borderWidth");
      const fillColor = this.getOption(chart, "point", "backgroundColor");
      ...
      chart.ctx.fillStyle = fillColor ? fillColor : "white";
      chart.ctx.lineWidth = borderWidth ? borderWidth : 2;
      chart.ctx.strokeStyle = borderColor ? borderColor : dataset.borderColor;
      chart.ctx.fill();
      chart.ctx.stroke();
    }
  },
